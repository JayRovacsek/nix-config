{ self, ... }:
{
      ...
      (
        ...
        prev.${version}.override {
          packageOverrides = python-final: python-prev: {
            ...
            afdko = python-prev.afdko.overrideAttrs (
              _:
              prev.buildPythonPackage rec {
                ...
                disabledTests =
                  ...
                  ++ prev.lib.optionals
                    (prev.stdenv.hostPlatform.isAarch || prev.stdenv.hostPlatform.isRiscV)
                    [
                      # unknown reason so far
                      # https://github.com/adobe-type-tools/afdko/issues/1425
                      "test_spec"
                    ]
                  ++ prev.lib.optionals prev.stdenv.hostPlatform.isi686 [ "test_type1mm_inputs" ];
            ...
            charset-normalizer = python-prev.charset-normalizer.overrideAttrs (_old: rec {
              version = "3.2.0";
              src = prev.fetchFromGitHub {
                owner = "Ousret";
            ...
            email-validator = python-prev.email-validator.overrideAttrs (old: rec {
              version = "2.0.0";
              src = prev.fetchFromGitHub {
                owner = "JoshData";
            ...
            flask-sqlalchemy = python-prev.flask-sqlalchemy.overrideAttrs (old: rec {
              version = "3.0.5";
              src = prev.fetchPypi {
              ...
              propagatedBuildInputs =
                old.propagatedBuildInputs ++ (with python-final; [ flit-core ]);
            });
            ...
            importlib-resources = python-prev.importlib-resources.overrideAttrs (_old: rec {
              version = "6.0.0";
              src = prev.fetchPypi {
            ...
            pyxattr = python-prev.pyxattr.overrideAttrs (old: rec {
              ...
              hardeningDisable = prev.lib.optionals prev.stdenv.isDarwin [ "strictoverflow" ];
            });
            ...
            sqlalchemy = python-prev.sqlalchemy.overrideAttrs (_old: rec {
              ...
              src = prev.fetchFromGitHub {
                owner = "sqlalchemy";
                repo = "sqlalchemy";
                rev = "refs/tags/rel_${prev.lib.replaceStrings [ "." ] [ "_" ] version}";
                hash = "sha256-97q04wQVtlV2b6VJHxvnQ9ep76T5umn1KI3hXh6a8kU=";
              };
            });

            sqlalchemy-utils = python-prev.sqlalchemy-utils.overrideAttrs (_old: rec {
              version = "0.41.1";
              src = prev.fetchPypi {
                inherit version;
  ...
  armv6l-fixes =
    ...
    let
      ...
      llvm-fixes =
        let
          llvm = prev.llvmPackages.llvm.overrideAttrs (_: {
            doCheck = false;
          });
        in
        {
          llvmPackages = prev.lib.recursiveUpdate prev.llvmPackages { inherit llvm; };
        };
            ...
            prev.${version}.override {
              packageOverrides = _: python-prev: {
                ...
                sphinxcontrib-jquery = python-prev.sphinxcontrib-jquery.overrideAttrs (old: {
                  propagatedBuildInputs =
                    old.propagatedBuildInputs ++ (with python-prev; [ sphinx ]);
                });

                readthedocs-sphinx-ext =
                  python-prev.readthedocs-sphinx-ext.overrideAttrs
                    (old: {
                      propagatedBuildInputs =
                        old.propagatedBuildInputs
                        ++ (with python-prev; [
    ...
    sonarr = prev.sonarr.overrideAttrs (old: {
      installPhase = ''
        ...
        makeWrapper "${prev.dotnet-runtime}/bin/dotnet" $out/bin/NzbDrone \
          ...
          --prefix PATH : ${
            prev.lib.makeBinPath [ (prev.ffmpeg.override { withSdl2 = false; }) ]
          } \
