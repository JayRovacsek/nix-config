...
let
  ...
  #    in the partitioner, so on the `diskoImages` path the very first command
  #    aborted the (set -e) build before the partitioner ever ran. We instead
  #    mount binfmt_misc if needed and write the registration line directly to
  ...
  binfmtSetup =
    lib.optionalString (cfg.enableBinfmt && binfmt.systemsAreDifferent)
      ''
        ${pkgs.kmod}/bin/modprobe binfmt_misc 2>/dev/null || true
        if [ ! -e /proc/sys/fs/binfmt_misc/register ]; then
          mount -t binfmt_misc binfmt_misc /proc/sys/fs/binfmt_misc
        fi
        ${pkgs.coreutils}/bin/printf '%s\n' ${lib.strings.escapeShellArg binfmt.binfmtRegistration} > /proc/sys/fs/binfmt_misc/register
      '';
  ...
  vmTools = pkgs.vmTools.override (
    {
      ...
      kernel = pkgs.aggregateModules (
        ...
        ++ lib.optional (
          cfg.kernelPackages.kernel ? modules
        ) cfg.kernelPackages.kernel.modules
        ++ lib.optional (
  ...
  partitioner = ''
    ...
    ln -sfn ${
      systemToInstallNative.config.environment.etc."udev/rules.d".source
    } /etc/udev/rules.d
    mkdir -p /dev/.mdadm
...
in
{
  system.build.diskoImages = lib.mkForce (
  ...
  system.build.diskoImagesScript = lib.mkForce (
    diskoLib.writeCheckedBash { inherit checked pkgs; } cfg.name ''
      ...
      * --build-memory <amt>
      ...
      export postVM=${
        diskoLib.writeCheckedBash { inherit pkgs checked; } "postVM.sh" cfg.extraPostVM
      }
