...
in {
  ...
  nodes = {
    ...
    pfsense = mkRouter "pfsense" {
      ...
      interfaces = let
        vlan-interfaces = builtins.foldl' (acc: vlan:
          recursiveUpdate acc {
            ${vlan.name} = {
              network = vlan.name;
  ...
  networks = let
    vlan-networks = builtins.foldl' (acc: vlan:
      recursiveUpdate acc {
        ${vlan.name} = {
          inherit (vlan) name;
