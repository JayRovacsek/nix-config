...
let
  ...
  buildSystemFunction = system: {
    ...
    hostName = "${system.networking.hostName}.${
        if builtins.hasAttr "localDomain" system.networking then system.networking.localDomain else "lan"
      }";
    ...
    speedFactor = builtins.mul (hardwareProfile system).cores (hardwareProfile system).speed;
  };

  buildMachines = builtins.filter (x: x.speedFactor != 1)
    (builtins.map (host: buildSystemFunction host.config) (builtins.attrValues flake.nixosConfigurations));
  ...
  localDomain =
    if hasAttr "localDomain" config.networking then config.networking.localDomain else config.networking.domain;

  darwinHosts = if hasAttr "darwinConfigurations" flake then attrNames flake.darwinConfigurations else [ ];

  linuxHosts = if hasAttr "nixosConfigurations" flake then attrNames flake.nixosConfigurations else [ ];

  extraHostNames = darwinHosts ++ linuxHosts;

  requireBuilderConfigs = (hasAttr "builder" config.users.users) && (hasAttr "builder-id-ed25519" config.age.secrets);

  extraHostConfigs = let fqdn = hostName: if localDomain == null then "${hostName}" else "${hostName}.${localDomain}";
...
in {
  ...
  nix = {
    ...
    settings = {
      ...
      sandbox = true;
      substituters = [ "https://binarycache.rovacsek.com/" "https://microvm.cachix.org/" ];
      trusted-public-keys = [
