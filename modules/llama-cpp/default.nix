{ config, lib, ... }: {
  # Simple kludge around what seems to be broken settings upstream
  ...
  services.llama-cpp = {
    ...
    settings = {
      ...
      models-dir = "/srv/storage/models";

      # Router server: limit concurrent loaded models to save VRAM
      # Load 1 model at a time, auto-swap on demand
      models-max = 1;
      ...
      # ── API ───────────────────────────────────────────────────────────
      # enable jinja2 chat template support
      jinja = true;
      ...
      # quantised KV cache (Q4_0) to save VRAM
      cache-type-k = "q4_0";
      # quantised KV cache (Q4_0) to save VRAM
      cache-type-v = "q4_0";
      ...
      # avoid memory mapping to save host RAM
      no-mmap = true;
      # lock memory into RAM to avoid swapping
      mlock = true;

      models-preset = lib.generators.toINI { } {
        # gemma-4-E2B = {
